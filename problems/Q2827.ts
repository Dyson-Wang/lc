...
function calc(high: number, k: number): number {
    ...
    const memo: number[][][] = new Array(n).fill(0).map(() => new Array(k).fill(0).map(() => new Array(n * 2 + 1).fill(-1))); //n*k*(2n+1) 初始化为-1，表示未计算过

    return dfs(0, 0, n, true, false, k, s, memo);
}

function dfs(i: number, val: number, diff: number, isLimit: boolean, isNum: boolean, k: number, s: string, memo: number[][][]): number {
    if (i === s.length) {//0执行
        return isNum && val === 0 && diff === s.length ? 1 : 0; // 找到了一个合法数字
    }
    if (!isLimit && isNum && memo[i][val][diff] !== -1) {//不执行
    ...
    for (let d: number = isNum ? 0 : 1; d <= up; d++) { // 枚举要填入的数字 d
